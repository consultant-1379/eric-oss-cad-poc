...
final String FLOW_STEP = "Get Baseline KPI data"
...
Reporter.updateReportSummary(execution, arcFlowUtils.getMessage(execution, "baselineKpi.title"))

final String ARC_OPTIMIZER_HOST = "arcOptimizer.host"
final String KPI_DATA_PATH = "kpiData.path"
String baselineKpiRequestBody =
...
try {
    apacheHttpClient.processPostRequest(baselineKpiRequestBody)
} catch (BpmnError bpmnError) {
...
if (responseCode == HttpStatus.SC_OK) {
    ...
    try {
        ...
        String message = arcFlowUtils.getMessage(execution, "kpi.error.unsuccessfulStartRequest")
    ...
    if (kpisDataMap.isEmpty()) {
        String errorMessage = arcFlowUtils.getMessage(execution, "kpi.error.emptyKpiData")
        EventRecorder.error(execution, errorMessage, FLOW_STEP)
        ...
        String message = arcFlowUtils.getMessage(execution, "kpi.status.successfulRetrieveRequest")
        LOGGER.info(message)
        EventRecorder.info(execution, message, FLOW_STEP)
    }
} else {
    String errorMessage = arcFlowUtils.getMessage(execution, "kpi.error.failedRetrieveRequest")
    EventRecorder.error(execution, errorMessage, FLOW_STEP)
    LOGGER.error("$errorMessage: $responseCode")
    throw new BpmnError(flowAutomationErrorCodes.ERROR_ARC_AUTOMATION_EXCEPTION, "$errorMessage: $responseCode")
}
