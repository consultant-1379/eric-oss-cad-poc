...
mandatoryGNodeBPairsSelection = execution.getVariable("mandatoryGNodeBPairsSelection") as Map
List<Map<String, Object>> mandatoryPairs = execution.getVariable("mandatoryPairs") as List
primaryGNBDU = mandatoryGNodeBPairsSelection.primaryGNBDUMandatory as List
secondaryGNBDU = mandatoryGNodeBPairsSelection.neighbouring as List
      ...
      sGnb ->
        if (pGnb == sGnb) {
          throw new UsertaskInputProcessingError(
              ...
              arcFlowUtils.getMessage(execution, "mandatoryGNodeBPairValidation.repeatedGNodeB") +
        ...
        if (excludedPairs.contains(newLink)) {
          throw new UsertaskInputProcessingError(
              flowAutomationErrorCodes.ERROR_USER_TASK_INVALID_INPUT,
              arcFlowUtils
                  .getMessage(execution, "mandatoryGNodeBPairValidation.pairAlreadyExcluded") +
                  newLink.toString()
          )
        }
        if (mandatoryPairs.contains(newLink)) {
          mandatoryPairs.remove(newLink)
        } else if (mandatoryPairs.stream().filter(
            { mandatoryLink -> mandatoryLink.pGnbduId == newLink.pGnbduId }).count() >= 6) {
          throw new UsertaskInputProcessingError(
              flowAutomationErrorCodes.ERROR_USER_TASK_INVALID_INPUT,
              arcFlowUtils.getMessage(execution, "mandatoryGNodeBPairValidation.tooManyMandatory") +
                  " For the gNodeB ID : " + newLink.pGnbduId.toString() + " ; gNodeB Name : " +
                  newLink.pGnbduName.toString()
          )
        } else if (mandatoryPairs.stream().filter(
            { mandatoryLink -> mandatoryLink.sGnbduId == newLink.sGnbduId }).count() >= 6) {
          throw new UsertaskInputProcessingError(
              flowAutomationErrorCodes.ERROR_USER_TASK_INVALID_INPUT,
              arcFlowUtils.getMessage(execution, "mandatoryGNodeBPairValidation.tooManyMandatory") +
                  " For the gNodeB ID : " + newLink.sGnbduId.toString() + " ; gNodeB Name : " +
                  newLink.sGnbduName.toString()
          )
        } else {
          mandatoryPairs.add("pGnbduId": pGnb["gNBId"],
              "pGnbduName": pGnb["gNBName"],
              "sGnbduId"  : sGnb["SecondarygNBId"],
              "sGnbduName": sGnb["SecondarygNBName"])
        }
